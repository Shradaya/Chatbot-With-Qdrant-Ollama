...
from src.utils.bert import get_bert_score
from src.utils.rouge import get_rouge_score
from src.llm.ollamaModels import llm, embedder
from src.qdrant_utils.connection import qdrant_connection
from src.utils.document_handler import get_text_from_document
...
def perform_context_retrieval(conn: qdrant_connection, question: str, use_rerank: bool = True, return_retrieve_interval: bool = False):
    ...
    contexts_ = [multi_replace(x.payload[ollama_configs.answer_key]) for x in retrieved_documents]
    retrieve_complete = datetime.now()
    
    
    contexts = conn.rerank_documents(question, contexts_) if use_rerank else contexts_
    rerank_complete = datetime.now()
    
    for index in range(len(contexts)):
        rank.append(contexts_.index(contexts[index]))
    ...
    if return_retrieve_interval:
        ...
        return contexts, database_retrieval_delta, rerank_delta, rank
    else:
        return contexts
...
def main():
    ...
    if args.ui:
        ...
        with open(output_file, 'w') as file:
            ...
            file.write(f"Question, Database Retrieval Delta, Invoke Model Delta, Rerank Delta, {', '.join(titles)}, context rank, Answer, Generated Answer, Check Answer, precision, recall, fmeasure\n")
        ...
        # for i in QUESTIONS: # tqdm(QUESTIONS):
            ...
            # # SEARCH IN DATABASE      
            contexts, database_retrieval_delta, rerank_delta, rank = perform_context_retrieval(conn, question, return_retrieve_interval=True)
            ...
            answer_check = check(context_str, generated_answer, question)
            score = get_rouge_score(generated_answer, answer)
            # score = get_bert_score(generated_answer, answer)
            print(score)
            ...
            with open(output_file, 'a') as file:
                ...
                file.write(f"{multi_replace(question)}, {database_retrieval_delta}, {invoke_model_delta}, {rerank_delta}, {str(rank).replace(',', ';')}, {', '.join(contexts)}, {multi_replace(answer)}, {multi_replace(generated_answer)}, {answer_check}, {score.rouge1.precision}, {score.rouge1.recall}, {score.rouge1.fmeasure}\n")
                # file.write(f"{multi_replace(question)}, {database_retrieval_delta}, {invoke_model_delta}, {rerank_delta}, {str(rank).replace(',', ';')}, {', '.join(contexts)}, {multi_replace(answer)}, {multi_replace(generated_answer)}, {answer_check}, {score.precision}, {score.recall}, {score.fmeasure}\n")
